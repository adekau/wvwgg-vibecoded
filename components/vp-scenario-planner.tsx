...
export function VPScenarioPlanner({ matchId, match }: VPScenarioPlannerProps) {
  const [mode, setMode] = useState<'automated' | 'interactive'>('automated')
  const [desiredFirst, setDesiredFirst] = useState<TeamColor>('red')
  const [desiredSecond, setDesiredSecond] = useState<TeamColor>('blue')
  const [desiredThird, setDesiredThird] = useState<TeamColor>('green')
  ...
  const desiredOutcome = useMemo(
    () => ({
      first: desiredFirst,
      second: desiredSecond,
      third: desiredThird,
    }),
    [desiredFirst, desiredSecond, desiredThird]
  )
  ...
  const handleFirstChange = (value: TeamColor) => {
    setDesiredFirst(value)
    setResult(null)
  }

  const handleSecondChange = (value: TeamColor) => {
    setDesiredSecond(value)
    setResult(null)
  }

  const handleThirdChange = (value: TeamColor) => {
  ...
  // Interactive mode: Calculate projected VP based on current assignments
  ...
  const handlePlacementChange = useCallback(
    (skirmishId: number, team: TeamColor, placement: Placement) => {
      setAssignments((prev) => {
      ...
      <div className="p-6">
        ...
        <div className="flex items-center justify-between mb-6 flex-wrap gap-4">
          ...
          <div className="flex items-center gap-2">
            ...
            <div className="flex items-center gap-1 p-1 rounded-lg bg-muted">
              <Button
                variant={mode === 'automated' ? 'secondary' : 'ghost'}
                size="sm"
              ...
              <Button
                variant={mode === 'interactive' ? 'secondary' : 'ghost'}
                size="sm"
            ...
            {mode === 'interactive' && (
              <>
                ...
                <Button variant="outline" size="sm" onClick={handleShare} title="Share scenario">
        ...
        <div className="text-sm text-muted-foreground mb-6">
          {mode === 'automated' ? (
            <>
              ...
              {remainingCount > 0 && (
                <span className="block mt-1">
                  {remainingCount} skirmish{remainingCount !== 1 ? 'es' : ''} remaining in this
                  match.
                </span>
              )}
            </>
            ...
            <>
              Manually assign skirmish placements to explore different scenarios. Watch the
              projections update in real-time as you make changes.
            </>
          )}
        </div>
        ...
        <div className="mb-6 p-4 rounded-lg bg-muted/30">
          ...
          <div className="grid grid-cols-3 gap-3">
            {[currentStandings.first, currentStandings.second, currentStandings.third].map(
              (color, index) => {
                ...
                return (
                  ...
                  >
                    ...
                    <div className="text-xs font-mono mt-1">{currentVP[color]} VP</div>
                  </div>
                )
              }
            )}
        ...
        <div className="mb-6">
          <div className="flex items-center justify-between mb-3">
            <h3 className="text-sm font-medium">
              {mode === 'automated' ? 'Desired Final Standings' : 'Target Outcome'}
            </h3>
            {mode === 'interactive' && (
              <Button
                variant="outline"
          ...
          {mode === 'interactive' && autoFillMessage && (
          ...
          <div className="grid grid-cols-3 gap-3">
            <div>
              ...
              <select
                value={desiredFirst}
                onChange={(e) => handleFirstChange(e.target.value as TeamColor)}
            ...
            <div>
              ...
              <select
                value={desiredSecond}
                onChange={(e) => handleSecondChange(e.target.value as TeamColor)}
                className="w-full text-sm border rounded px-3 py-2 bg-background"
              >
                {colors.map((color) => {
                  const world = match.worlds.find((w) => w.color === color)
            ...
            <div>
              ...
              <select
                value={desiredThird}
                onChange={(e) => handleThirdChange(e.target.value as TeamColor)}
                className="w-full text-sm border rounded px-3 py-2 bg-background"
              >
                {colors.map((color) => {
                  const world = match.worlds.find((w) => w.color === color)
                  ...
                  <div className="mt-4 p-3 bg-muted/30 rounded-lg">
                    ...
                    <div className="space-y-2">
                      {result.solverAttempts.map((attempt, index) => (
                        <div
                          key={index}
                          className="text-xs flex items-center justify-between py-1 border-b border-border/50 last:border-0"
                        >
                          <div className="flex items-center gap-2">
                            <span
                      ...
                      <table className="w-full text-xs">
                        <thead className="sticky top-0 bg-background">
                          <tr className="border-b border-border/50">
                            ...
                            {match.worlds.map((world) => (
                              <th
                                key={world.color}
                                className={`text-center py-2 px-2 ${colorClasses[world.color].text}`}
                              >
                                {world.name}
                              </th>
                            ))}
                        ...
                        <tbody>
                          {result.requiredPlacements.map((placement) => {
                            const skirmish = remainingSkirmishes.find(
                              (s) => s.id === placement.skirmishId
                            )
                            ...
                            return (
                              <tr key={placement.skirmishId} className="border-b border-border/50">
                                ...
                                <td className="py-2 px-2 text-center font-mono">
                                  {skirmish.vpAwards.first}/{skirmish.vpAwards.second}/
                                ...
                                {match.worlds.map((world) => {
                                  ...
                                  return (
                                    <td key={world.color} className="py-2 px-2 text-center">
                                      <span
                                        className={`inline-block px-2 py-1 rounded ${placeBg} font-semibold`}
                                      >
                                        {placeText}
                                      </span>
                                    </td>
        ...
        {/* Interactive Mode: Live Projection and Assignment Table */}
        {mode === 'interactive' && (
          <>
            {/* Live Projection */}
            <div
            ...
            >
              <div className="flex items-center gap-2 mb-3">
                {outcomeStatus.achieves ? (
                  <>
                    <CheckCircle2 className="h-5 w-5 text-green-600 dark:text-green-400" />
                    <h3 className="font-semibold text-green-600 dark:text-green-400">On Track!</h3>
                  </>
                ) : (
                  <>
              ...
              <div>
                <span className="font-medium">Assigned:</span>{' '}
                {
                  Array.from(assignments.values()).filter(
                    (a) =>
                      a.placements.red !== null &&
                      a.placements.blue !== null &&
                      a.placements.green !== null
                  ).length
                }{' '}
                / {remainingSkirmishes.length}
              </div>
              <div>
                <span className="font-medium">1st-2nd Gap:</span>{' '}
                {outcomeStatus.vpGaps.firstSecond > 0 ? '+' : ''}
                {outcomeStatus.vpGaps.firstSecond} VP
              </div>
              <div>
                <span className="font-medium">2nd-3rd Gap:</span>{' '}
                {outcomeStatus.vpGaps.secondThird > 0 ? '+' : ''}
                {outcomeStatus.vpGaps.secondThird} VP
              </div>
            </div>
          </>
        )}
...
function decodeScenario(encoded: string): Map<number, SkirmishAssignment> {
  try {
